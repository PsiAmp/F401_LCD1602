...
#define LCD_MAX_DELAY 100
#define LCD_MAX_ATTEMPTS 3
...
HAL_StatusTypeDef LCD_SendInternal(I2C_HandleTypeDef *hi2c, uint8_t lcd_addr, uint8_t data, uint8_t flags) {
    ...
    while (attempts < LCD_MAX_ATTEMPTS) {
        res = HAL_I2C_IsDeviceReady(hi2c, lcd_addr, 1, LCD_MAX_DELAY);

        if(res == HAL_OK)
            break;

        attempts++;
        if (attempts == LCD_MAX_ATTEMPTS)
        	return HAL_TIMEOUT;
    }
    ...
    res = HAL_I2C_Master_Transmit(hi2c, lcd_addr, data_arr, sizeof(data_arr), LCD_MAX_DELAY);
...
void LCD_SendStringToLine(I2C_HandleTypeDef *hi2c, uint8_t lcd_addr, char *str, LCD_Line line) {
    LCD_SendCommand(hi2c, lcd_addr, line);
    LCD_SendString(hi2c, lcd_addr, str);
}
